...
#include <set>
#include <vector>
static const char *DEFAULT_LABEL_PATH = "./model/coco_80_labels_list.txt";
static const char *SINGLE_CLASS_LABEL = "person";
static std::vector<char *> labels;
static int g_obj_class_num = 0;
...
static int loadLabelName(const char *locationFilename, std::vector<char *> &label, int class_num)
{
    printf("load lable %s\n", locationFilename);
    if (class_num <= 0)
    {
    ...
    label.resize(class_num, nullptr);
    readLines(locationFilename, label.data(), class_num);
    return 0;
}
...
{
    g_obj_class_num = class_num;
    labels.clear();

    if (g_obj_class_num <= 0)
    {
        return -1;
    }

    if (g_obj_class_num == 1)
    {
        labels.resize(1, nullptr);
        labels[0] = strdup(SINGLE_CLASS_LABEL);
        return labels[0] ? 0 : -1;
    }

    int ret = loadLabelName(DEFAULT_LABEL_PATH, labels, g_obj_class_num);
    if (ret < 0)
    {
...
const char *coco_cls_to_name(int cls_id)
{
    ...
    if (cls_id < (int)labels.size() && labels[cls_id])
    {

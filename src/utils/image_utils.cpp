...
int convert_image(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    (void)dst_box;
    (void)color;
    fprintf(stderr, "[rga] image preprocessing unavailable: this build has no RGA support\n");
    return -1;
#else
    ...
    if (src_img->width % alignment != 0 || dst_img->width % alignment != 0) {
        fprintf(stderr,
                "[rga] image preprocessing rejected: width alignment=%d src=%d dst=%d\n",
                alignment,
                src_img->width,
                dst_img->width);
        return -1;
    ...
    const int ret = convert_image_rga(src_img, dst_img, src_box, dst_box, color);
    if (ret != 0) {
        fprintf(stderr,
                "[rga] image preprocessing failed: ret=%d src=%dx%d dst=%dx%d\n",
                ret,
                src_img->width,
                src_img->height,
                dst_img->width,
